...
function initializeApp() {
    ...
    if (locationParam) {
        ...
        showMessage('Please enter a location to continue.', 'warning');
    }
...
async function handleFormSubmit(e) {
    ...
    if (description.length < 10) {
        showFieldError(descriptionInput, 'Please provide a more detailed description (at least 10 characters)');
...
function handleSubmissionError(error) {
    const message = error.message || 'An error occurred while submitting your report. Please try again.';
...
function clearMessages() {
    if (messageArea.classList.contains('error') || messageArea.classList.contains('warning')) {
        messageArea.style.display = 'none';
    }
}
...
window.addEventListener('offline', () => {
    showMessage('⚠ You are currently offline. Your report will be submitted when connection is restored.', 'warning');
});
